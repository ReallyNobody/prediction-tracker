...
class PredictionMarketAnalyzer:
    ...
    def create_probability_comparison(
        self, output_path="/home/claude/prediction_probabilities.png"
    ):
        ...
        for m in self.data["predictit"]:
            markets_data.append(
                {
                    "title": m["market_name"][:50] + "..."
                    if len(m["market_name"]) > 50
                    else m["market_name"],
                    "probability": m["yes_price"] * 100,
                    "platform": "PredictIt",
        ...
        y_pos = range(len(df))
        colors = [
            self.brand_colors.get(p.lower(), self.brand_colors["primary"]) for p in df["platform"]
        ]
        ...
        legend_elements = [
            ...
            plt.Rectangle(
                (0, 0), 1, 1, fc=self.brand_colors["predictit"], alpha=0.8, label="PredictIt"
            ),
            plt.Rectangle(
                (0, 0), 1, 1, fc=self.brand_colors["polymarket"], alpha=0.8, label="Polymarket"
            ),
        ]
        ...
        fig.text(
            0.99,
            0.01,
            "Risk Market News | Source: Kalshi, PredictIt, Polymarket",
            ha="right",
            fontsize=9,
            style="italic",
            color="gray",
    ...
    def create_volume_analysis(self, output_path="/home/claude/market_volume.png"):
        ...
        ax1.pie(
            platform_volumes.values(),
            labels=platform_volumes.keys(),
            autopct="%1.1f%%",
            colors=colors,
            startangle=90,
        ...
        ax1.set_title(
            "Trading Volume by Platform\n(Climate/Insurance Markets)", fontsize=14, weight="bold"
        )
        ...
        bars = ax2.barh(
            range(len(kalshi_df)),
            kalshi_df["volume"] / 1000,
            color=self.brand_colors["kalshi"],
            alpha=0.8,
        )
        ax2.set_yticks(range(len(kalshi_df)))
        ax2.set_yticklabels(
            [t[:40] + "..." if len(t) > 40 else t for t in kalshi_df["title"]], fontsize=9
        )
        ax2.set_xlabel("Trading Volume (thousands)", fontsize=11, weight="bold")
        ax2.set_title(
            "Market Trading Volume\nKalshi Climate/Insurance Markets", fontsize=14, weight="bold"
        )
        ...
        for i, vol in enumerate(kalshi_df["volume"]):
            ax2.text(
                vol / 1000 + 2, i, f"${vol / 1000:.0f}K", va="center", fontsize=10, weight="bold"
            )
        ...
        fig.text(
            0.99,
            0.01,
            "Risk Market News | Source: Prediction Markets",
            ha="right",
            fontsize=9,
            style="italic",
            color="gray",
    ...
    def create_event_risk_dashboard(self, output_path="/home/claude/event_risk_dashboard.png"):
        ...
        ax1.barh(range(len(high_risk)), high_risk["yes_price"] * 100, color="#ef4444", alpha=0.8)
        ...
        ax2.scatter(
            poly_df["liquidity"] / 1e6,
            poly_df["volume"] / 1e6,
            s=200,
            alpha=0.6,
            c=self.brand_colors["polymarket"],
        )
        ...
        colors_map = [
            "#ef4444" if p > 0.5 else "#f59e0b" if p > 0.3 else "#10b981" for p in events_df["prob"]
        ]
        ...
        fig.suptitle(
            "🌪️ Catastrophe Risk Intelligence Dashboard\nPrediction Markets + Insurance Data",
            fontsize=18,
            weight="bold",
            y=0.98,
        )
        ...
        fig.text(
            0.99,
            0.01,
            "Risk Market News | Real-time prediction market data",
            ha="right",
            fontsize=9,
            style="italic",
            color="gray",
    ...
    def generate_newsletter_summary(self):
        ...
        for m in sorted_markets[:3]:
            ...
            summary.append(
                f"    Volume: ${m['volume']:,} | Probability: {m['yes_price'] * 100:.0f}%"
            )
