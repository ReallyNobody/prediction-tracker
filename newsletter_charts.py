...
class NewsletterChartGenerator:
    ...
    def create_event_comparison_chart(self, data, output_path='/home/claude/event_comparison.png'):
        ...
        # Aggregate by event
        event_totals = data.groupby('event_name').agg({
            'net_loss_usd': 'sum',
            'gross_loss_usd': 'sum'
        ...
        # Create bars
        bars1 = ax.barh(y_pos, event_totals['gross_loss_usd'] / 1e6, 
                       color=self.brand_colors['primary'], alpha=0.6, label='Gross Loss')
        bars2 = ax.barh(y_pos, event_totals['net_loss_usd'] / 1e6,
                       color=self.brand_colors['secondary'], alpha=0.9, label='Net Loss')
        
        # Formatting
        ax.set_yticks(y_pos)
        ax.set_yticklabels(event_totals.index, fontsize=11, weight='bold')
        ax.set_xlabel('Loss (USD Millions)', fontsize=12, weight='bold')
        ax.set_title('Catastrophe Losses by Event\nGross vs Net Retained', 
                    fontsize=16, weight='bold', pad=20)
        
        # Add value labels
        for i, (gross, net) in enumerate(zip(event_totals['gross_loss_usd'], 
                                             event_totals['net_loss_usd'])):
            ax.text(gross / 1e6 + 10, i, f'${gross/1e6:.0f}M', 
                   va='center', fontsize=10, color=self.brand_colors['dark'])
            ax.text(net / 1e6 + 10, i - 0.15, f'${net/1e6:.0f}M', 
                   va='center', fontsize=10, weight='bold', color=self.brand_colors['secondary'])
        
        ax.legend(loc='lower right', fontsize=11, framealpha=0.9)
        ax.grid(axis='x', alpha=0.3)
        
        # Add branding
        fig.text(0.99, 0.01, 'Risk Market News | Source: SEC Filings', 
    ...
    def create_company_market_share_chart(self, data, output_path='/home/claude/company_market_share.png'):
        ...
        colors = [self.brand_colors['primary'], self.brand_colors['secondary'], 
                 self.brand_colors['accent'], '#f59e0b', '#8b5cf6']
        
        wedges, texts, autotexts = ax.pie(company_totals, 
        ...
        ax.set_title('Net Catastrophe Losses by Company\nMarket Share of Reported Losses', 
                    fontsize=16, weight='bold', pad=20)
        
        # Add legend with dollar amounts
        legend_labels = [f'{ticker}: ${amt/1e6:.0f}M' 
                        for ticker, amt in company_totals.items()]
    ...
    def create_timeline_chart(self, data, output_path='/home/claude/loss_timeline.png'):
        ...
        colors = [self.brand_colors['primary'], self.brand_colors['secondary'],
                  self.brand_colors['accent']] * (len(timeline_data) // 3 + 1)
        scatter = ax.scatter(timeline_data['event_date'], 
        ...
        ax.set_title('Catastrophe Loss Timeline\nBubble size represents gross loss', 
                    fontsize=16, weight='bold', pad=20)
    ...
    def create_retention_ratio_chart(self, data, output_path='/home/claude/retention_ratios.png'):
        ...
        company_data['retention_ratio'] = (company_data['net_loss_usd'] / 
                                           company_data['gross_loss_usd'] * 100)
        company_data = company_data.sort_values('retention_ratio', ascending=True)
        
        fig, ax = plt.subplots(figsize=(12, 6), facecolor='white')
        
        y_pos = range(len(company_data))
        bars = ax.barh(y_pos, company_data['retention_ratio'],
                      color=[self.brand_colors['primary'] if x < 40 else 
                             self.brand_colors['accent'] if x < 50 else 
        ...
        ax.set_title('Catastrophe Loss Retention Ratios by Company\nNet Loss as % of Gross Loss', 
                    fontsize=16, weight='bold', pad=20)
        
        # Add value labels
        for i, (ratio, net, gross) in enumerate(zip(company_data['retention_ratio'],
                                                    company_data['net_loss_usd'],
                                                    company_data['gross_loss_usd'])):
            ax.text(ratio + 1, i, f'{ratio:.1f}%', va='center', fontsize=11, weight='bold')
            ax.text(ratio - 5, i, f'${net/1e6:.0f}M / ${gross/1e6:.0f}M', 
