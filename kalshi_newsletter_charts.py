...
class KalshiNewsletterCharts:
    ...
    def create_volume_chart(self, output_path='kalshi_top_volume.png'):
        ...
        top10['short_title'] = top10['short_title'].apply(lambda x: x[:60] + '...' if len(x) > 60 else x)
        
        # Create bars
        colors = [self.colors['primary'] if 'high temp' in t.lower() else 
                 self.colors['secondary'] if 'low temp' in t.lower() else 
                 self.colors['accent'] for t in top10['title']]
        ...
        ax.set_title('Top Weather Prediction Markets by Volume\nKalshi Trading Data', 
                    fontsize=17, weight='bold', pad=20)
        
        # Add value labels
        for i, (idx, row) in enumerate(top10.iterrows()):
            ax.text(row['volume_24h'] + 100, i, f"${row['volume_24h']:,.0f}", 
        ...
        # Branding
        fig.text(0.99, 0.01, 'Risk Market News | Source: Kalshi.com | ' + datetime.now().strftime('%B %d, %Y'),
    ...
    def create_city_comparison(self, output_path='kalshi_by_city.png'):
        ...
        city_stats = self.df.groupby('city').agg({
            ...
            'volume_24h': 'sum',
            'ticker': 'count'
        }).sort_values('volume_24h', ascending=False).head(12)
        
        # Create bars
        x_pos = range(len(city_stats))
        bars = ax.bar(x_pos, city_stats['implied_prob'], 
                     color=self.colors['secondary'], alpha=0.8, edgecolor='black', linewidth=1.5)
    ...
    def create_market_overview(self, output_path='kalshi_market_overview.png'):
        ...
        colors_type = [self.colors['primary'], self.colors['secondary'], 
                      self.colors['accent'], '#f59e0b']
        ax1.pie(type_counts, labels=type_counts.index, autopct='%1.0f%%',
               colors=colors_type, startangle=90, textprops={'fontsize': 11, 'weight': 'bold'})
        ...
        self.df['volume_bin'] = pd.cut(self.df['volume_24h'], bins=volume_bins, 
        ...
        ax3.hist(self.df['implied_prob'], bins=20, color=self.colors['accent'], 
        ...
        ax4.text(0.1, 0.5, stats_text, fontsize=12, family='monospace',
                ...
                facecolor='#f0f9ff', edgecolor=self.colors['primary'], linewidth=2))
        
        # Overall title
        fig.suptitle('Kalshi Weather Markets - Market Overview\nReal-time Prediction Market Intelligence', 
    ...
    def create_probability_heatmap(self, output_path='kalshi_temp_heatmap.png'):
        ...
        temp_df['temp'] = temp_df['title'].str.extract(r'(\d+)-(\d+)°').apply(
            lambda x: (int(x[0]) + int(x[1])) / 2 if x[0] and x[1] else None, axis=1)
        
        temp_df = temp_df.dropna(subset=['temp'])
        
        # Create pivot table
        pivot = temp_df.pivot_table(values='implied_prob', 
                                     index='city', 
                                     columns='temp', 
                                     aggfunc='mean')
